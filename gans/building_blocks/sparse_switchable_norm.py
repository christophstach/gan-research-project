...
class SparseSwitchNorm2d(nn.Module):
    def __init__(self, num_features, eps=1e-5, momentum=0.997, using_moving_average=True, last_gamma=False):
        super(SparseSwitchNorm2d, self).__init__()
        self.eps = eps
        ...
        # self.rad = 0.
        self.register_buffer('mean_fixed', torch.tensor([0]))
        self.register_buffer('var_fixed', torch.tensor([0]))
        self.register_buffer('radius', torch.zeros(1))

        self.mean_weight_ = torch.tensor([1., 1., 1.])
        self.var_weight_ = torch.tensor([1., 1., 1.])
