...
class StachGAN(pl.LightningModule):
    ...
    def generator_loss(self, fake_images):
        ...
        return F.binary_cross_entropy(fake_images, fake)
    ...
    def discriminator_loss(self, real_images, fake_images):
        ...
        return (F.binary_cross_entropy(self.discriminator(real_images).view(-1), real)
                + F.binary_cross_entropy(self.discriminator(fake_images).view(-1), fake)) / 2
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:  # Train discriminator
            ...
            fake_loss = self.generator(noise)
            loss = self.generator_loss(fake_loss)

            if batch_idx % 50 == 0:
                grid = torchvision.utils.make_grid(fake_loss[:6])
    ...
    def add_model_specific_args(parent_parser):
        ...
        train_group.add_argument("-acb", "--accumulate_grad_batches", type=int, default=4, help="Accumulate gradient batches")
